...
#define GAIN_KV1              1000.0
#define GAIN_KV2              1000.0
...
ArrayMatrix<2,1,float> matrix_wheelAngularVelocityCommand = { 0.0, 0.0};
...
void commandVelocityCallback(const geometry_msgs::Twist &msg)
{
  ...
  // v = r x w ==> r = WHEEL_SEPARATION / 2, w = cmd_angular_z
  // to go on left side => upper wheel right, down wheel left

  wheel_velocity[RIGHT] = cmd_linear_x + (cmd_angular_z * (WHEEL_SEPARATION / 2));
  wheel_velocity[LEFT]  = cmd_linear_x - (cmd_angular_z * (WHEEL_SEPARATION / 2));

  matrix_wheelAngularVelocityCommand(0,0) = wheel_velocity[RIGHT] * velocity_constant_value;
  matrix_wheelAngularVelocityCommand(1,0) = wheel_velocity[LEFT] * velocity_constant_value;
}
...
bool initWheelDynamixels(void)
{
  ...
  {
    // Set Torque to OFF
    result = dxl_wb.torqueOff((uint8_t)it->second, &log);
    if (result == false)
    {
      Serial.println(log);
      Serial.print("Failed to set Torque to OFF !");
      Serial.print((it->first).c_str());
      Serial.println(" !");
      Serial.print("ID : ");
      Serial.println((uint8_t)it->second);
    ...
    {
      Serial.println("Succeeded  to set Torque to OFF !");
      Serial.print((it->first).c_str());
      Serial.println(" !");
      Serial.print("ID : ");
      Serial.println((uint8_t)it->second);
