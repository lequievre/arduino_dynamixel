...
ros::Subscriber<std_msgs::Float32MultiArray> cmd_arm_position_sub("cmd_pos_arm", commandArmPositionCallback );
...
bool initArmDynamixels(void)
{
  ...
  while (it != map_id_arm_dynamixels.end())
  {
    result = dxl_wb.jointMode((uint8_t)it->second, 0, 0, &log);
    if (result == false)
    {
      Serial.println(log);
