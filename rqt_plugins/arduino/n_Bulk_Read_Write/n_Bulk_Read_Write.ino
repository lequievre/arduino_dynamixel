...
{  
  ...
  result = dxl_wb.addBulkWriteParam(dxl_id[1], "Goal_Position", goal_position[0], &log);
  if (result == false)
  {
    Serial.println(log);
    Serial.println("Failed to add bulk write position param");
  ...
  do
  {
    ...
    else
    {
      ...
      Serial.print(" Present Position : ");
      Serial.print(get_data[1]);
    }

  }while((abs(goal_position[0] - get_data[0]) > 15) && (abs(goal_position[0] - get_data[1]) > 15));
